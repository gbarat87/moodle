...
$string['check_installdir_details'] = '<p>The directory <code>{$a->path}</code> contains the Moodle installer and its files, which are only needed while installing or upgrading Moodle. They are not needed to run a Moodle site in production and they can expose information about your site configuration to remote attackers.</p><p>It is strongly recommended to remove the directory if the site is available via a public URL, or at least prohibit web access to it in your webserver configuration.</p>';
$string['check_installdir_info'] = 'The install directory should not be present on public sites.';
